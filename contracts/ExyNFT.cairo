%lang starknet #Seulement une ligne de code nécéssaire pour compiler notre contract starknet
...
@constructor
func constructor{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}( # Les arguments que nous devons renseigné lors
    name : felt,
    symbol : felt,
    owner : felt,
...
func setTokenURI{pedersen_ptr : HashBuiltin*, syscall_ptr : felt*, range_check_ptr}(
    ...
    Ownable.assert_only_owner() # OnlyOwner en solidity, uniquement l'owner du contract peut appeler ce setter
    ERC721_Metadata_setBaseTokenURI(base_token_uri_len, base_token_uri, token_uri_suffix) 
    return ()
end
...
func mint{pedersen_ptr : HashBuiltin*, syscall_ptr : felt*, range_check_ptr}():
    let (caller_address) = get_caller_address() # Nous récupérons l'adress du caller de la fonction
    let (tokenId: Uint256) = next_token.read() # Nous récupérons l'id next_token (storage_var)
    ERC721._mint(caller_address, tokenId) # Mint du token au caller 
    let (next_tokenId, _) = uint256_add(tokenId, Uint256(1, 0)) 
    next_token.write(next_tokenId) # Incrémentation du next_tokenId
    return ()
end
